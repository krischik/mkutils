...
--          $Id: ada_demo.ads 16 2007-10-31 17:08:47Z
...
--    $Revision$
--     $HeadURL:
--  https://mkutils.googlecode.com/svn/trunk/Source/ada_demo/ada_demo.ads $
--      History: 25.10.2007 MK Initial Release
--               29.10.2007 MK Added Threading, parameter names closer to
--                             C original
--         Help:
--  http://www.jpsoftwiki.com/wiki/index.php?title=Ada_Demo_%28plugin%29
----------------------------------------------------------------------------
...
--  verison 8. It would be sensible to recomplie them with the new PLUGININFO
--  structure that can be found in this demonstration plugin.
------------------------------------------------------------------------------
--
--  Introduction                                                        {{{2
--  ----------------
...
--  with 4NT and Take Command.
--
--  Using the 4NT/Take Command API with Ada                             {{{2
--  ------------------------------------------
--
--  Functions that are to be called by 4NT / TC need to be in the general
--  format:
--
...
--
--  Every plugin must define the PLUGININFO structure and return a pointer to
--  it via the GetPluginInfo function. Amongst other things the PLUGININFO
--  structure identifies to 4NT/TC the Internal Commands, Internal Variables
--  and Variable Functions that the plugin implements. More comments on
--  PLUGININFO can be found later in this file in the Type definitions.
--
--  Internal Variable names in the "Implements" field of the PLUGININFO
--  structure (and their corresponding function names) must begin with an
--  underscore ('_').
--
--  Variable Function names in the "Implements" field of the PLUGININFO
--  structure must begin with an @; the corresponding function must be
--  prefixed by "f_". (This allows variable functions to have the same name as
--  internal commands.)
--
--    For example:
--
...
--
--  Entering the name "reverse" on the command line will invoke the command
--  reverse()
--
--  Entering the name "@reverse[]" on the command line will invoke the
--  variable function f_reverse()
--
--  Variable function names are limited to a maximum of 31 characters.
--
--  Internal command names may be any combination of alphanumeric characters
--  up to a maximum of 12 characters.
--
--  The case of function names (for Internal Commands, Variable Functions and
--  Internal Variables) in the "Implements" field of PLUGININFO and the
--  corresponding function names must match exactly or 4NT/TC will not
--  recognise them (e.g. if you name a function "Sample" but put "sample" in
--  the Implements field of PLUGININFO it will not work).
--
...
--
--  4NT & Take Command will trap any exceptions occurring in the plugin, to
...
--
--  Note that all strings passed between 4NT/TC and the plugin are null
--  terminated and are UNICODE.
--
--  Version 8.0 of 4NT and Take Command introduced keystroke monitoring. A
...
--  specific keystrokes being pressed. A keystroke monitoring function's name
--  must be prefixed by "*" in the "Implements" field of the PLUGININFO record
--  to identify it to 4NT and Take Command. This demonstration plugin includes
--  a keystroke monitoring function.
--
...
private
   ...
   ------------------------------------------------------------------------
   --  Every function that you wish to make visible to 4NT/TC must be declared
   --  in the following exports clause. The first three declarations are
   --  essential - add you own functions after them. The case of the function
   --  names in the exports clause is not significant.
   --
   pragma Export
     (Convention => Stdcall,
      Entity => Initialize_Plugin,
   ...
   pragma Export
     (Convention => Stdcall,
      Entity => F_Reverse,
      External_Name => "f_REVERSE");
   pragma Export
     (Convention => Stdcall,
      Entity => V_Hello,
      External_Name => "_HELLO");
   pragma Export
     (Convention => Stdcall,
      Entity => C_Remark,
